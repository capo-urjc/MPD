...
class FastTemporalPollutionDataset(Dataset):
    def __init__(self, path: str, correspondences: dict, sq_len_to_train: int, sq_len_to_predict: int,
        ...
        self.sin_wind_dir = sin_wind_dir
        ...
        if self.normalization:
            for m in self.magnitudes:
                if m != 82:
                    ...
                    # [-1, 1]
                    csv_file.loc[csv_file["MAGNITUD"] == m, "VALOR"] = - 1 + 2*((csv_file[csv_file["MAGNITUD"] == m]["VALOR"] - min_pos_value) / (max_pos_value - min_pos_value))

        for m in self.magnitudes:
            csv_file.loc[csv_file["MAGNITUD"] == m, "VALOR"] = csv_file.loc[csv_file["MAGNITUD"] == m, "VALOR"].interpolate(method=self.interpolate)
        ...
        # Add sin and cosine codification
        if 82 in self.magnitudes_to_train:
            idx = self.magnitudes_to_train.index(82)
            values_x = np.insert(values_x, idx + 1, np.sin(np.radians(values_x[:, :, idx])), axis=2)
            values_x = np.insert(values_x, idx + 2, np.cos(np.radians(values_x[:, :, idx])), axis=2)
            values_x[:, :, idx] = -1 + 2 * (values_x[:, :, idx] - 0) / 360

        if 82 in self.magnitudes_to_predict:
            idx = self.magnitudes_to_predict.index(82)
            values_y = np.insert(values_y, idx + 1, np.sin(np.radians(values_y[:, :, idx])), axis=2)
            values_y = np.insert(values_y, idx + 2, np.cos(np.radians(values_y[:, :, idx])), axis=2)
            values_y[:, :, idx] = -1 + 2 * (values_y[:, :, idx] - 0) / 360
...
if __name__ == "__main__":
    ...
    magnitudes_to_train: list = [20]
    magnitudes_to_predict: list = [20]
    locations_to_train: list = [(16, 1), (47, 2), (58, 4), (6, 4)]
    locations_to_predict: list = [(16, 1), (47, 2), (58, 4), (6, 4)]  # [(5, 2), (171, 1)]
