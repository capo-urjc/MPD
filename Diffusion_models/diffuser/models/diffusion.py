...
#         loss_type='l1', clip_denoised=False, predict_epsilon=True,
#         loss_discount=1.0, loss_weights=None, n_locations=20,
#         fn_condition=apply_conditioning_NO2):
#
#         super().__init__()
#         self.horizon = horizon
#         self.observation_dim = observation_dim
...
#             rang = self.transition_dim//len(weights_dict)
#             for j in  range(rang):
#                 for ind, w in weights_dict.items():
#                     dim_weights[(j * len(weights_dict)) + ind] *= w
#
#         ## decay loss with trajectory timestep: discount**t
#         discounts = discount ** torch.arange(self.horizon, dtype=torch.float)
#         discounts = discounts / discounts.mean()
...
#
#         with torch.no_grad():
#             x_aux = x_recon.reshape((x_recon.shape[0], self.horizon,  self.n_locations, -1))
#             x_gt = x_start.reshape((x_start.shape[0], self.horizon, self.n_locations, -1))
#
#             for i in range(x_aux.shape[-1]):
#                  info['loss_'+str(i)] = torch.mean(torch.abs(x_aux[..., i] - x_gt[..., i]))
#         return loss, info
#
#     def loss(self, x, *args):
#         batch_size = len(x)
