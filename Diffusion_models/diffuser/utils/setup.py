...
class Parser(Tap):
    ...
    def mkdir(self, args):
        if 'logbase' in dir(args) and 'dataset' in dir(args) and 'exp_name' in dir(args):
            args.savepath = os.path.join(args.logbase, args.dataset, args.exp_name, datetime.now().strftime("%Y-%m-%d:%H-%M"))
...
class ParserNO2(Parser):
    def read_config(self, args, experiment):
        ...
        #     params.update(overrides)
        # else:
        print(f'[ utils/setup ] config: {args.config}')
    ...
    def mkdir(self, args):
        if 'logbase' in dir(args) and 'exp_name' in dir(args):
            args.savepath = os.path.join(args.logbase, args.exp_name, datetime.now().strftime("%Y-%m-%d:%H-%M"))
            self._dict['savepath'] = args.savepath
            if 'suffix' in dir(args):
                args.savepath = os.path.join(args.savepath, args.suffix)
            if mkdir(args.savepath):
